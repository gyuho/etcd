...
func (x *intervalNode) visit(iv *Interval, sentinel *intervalNode, nv nodeVisitor) bool {
	fmt.Println(x.iv.Ivl, x == sentinel, "visiting", iv, x.left.iv, x.right.iv)
	if x == sentinel {
		return false
	}
	...
	default:
		...
		if x.left == sentinel && x.right != sentinel {
			fmt.Println("1")
			if !x.right.visit(iv, sentinel, nv) || !nv(x) || !x.left.visit(iv, sentinel, nv) {
				fmt.Println("return false 1")
				return false
			}
			...
			fmt.Println("3")
			if !x.left.visit(iv, sentinel, nv) || !nv(x) || !x.right.visit(iv, sentinel, nv) {
				fmt.Println("return false 3")
				return false
			}
		} else {
			fmt.Println("4", x.left == sentinel, x.right == sentinel)
			a := x.left.visit(iv, sentinel, nv)
			b := nv(x)
			c := x.right.visit(iv, sentinel, nv)
			if !a || !b || !c {
				fmt.Println("return false 4")
				return false
			}
		}
	}
