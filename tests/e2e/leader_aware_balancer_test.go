...
func TestLeaderAwareBalancerFailoverE2E(t *testing.T) {
	...
	require.NotEqual(t, leadIdx, newLeadIdx, "leader did not change after stopping the old leader")
...
func TestLeaderAwareBalancerMoveLeaderE2E(t *testing.T) {
	...
	newLeadIdx := epc.WaitLeader(t)
	require.Equal(t, targetIdx, newLeadIdx, "leader was not transferred to the target member")
